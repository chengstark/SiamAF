...
DIM1, DIM2 = 512, 128
PREDICTOR = True
comment = 'bs=5000AdamNOCLIP'
MODEL_FOLDER = f'corrected_resnet_34_epoch_{NUM_EPOCHS}_ecglr_{ECG_LR}_ppglr_{PPG_LR}_lambda_{LAMBDA}_dim1_{DIM1}_dim2_{DIM2}_pred_{PREDICTOR}_{comment}'
...
def train_epoch(epoch_idx, model, cos_loss, ce_loss_fn, optimizer, train_loader, lambda_):
    with torch.autograd.set_detect_anomaly(True):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            p1, p2, z1, z2, ECG_pred, PPG_pred = model(ECG, PPG)
