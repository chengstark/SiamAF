...
class Dataset_ECG_limited_labeled(Dataset):
    ...
    def build_dataset(self):
        ...
        AF_ori_shape = self.AF_ECG.shape[0]
        PVC_ori_shape = self.PVC_ECG.shape[0]
        NSR_ori_shape = self.NSR_ECG.shape[0]


        print_flush(f'\tloading {self.split} finished t={datetime.now() - tstart}, mem used={psutil.virtual_memory()[3]/1000000000}')

        self.AF_subsample_idx = np.random.choice(range(self.AF_ECG.shape[0]), round(self.labeled_perc * self.AF_ECG.shape[0]), replace=False)
        self.PVC_subsample_idx = np.random.choice(range(self.PVC_ECG.shape[0]), round(self.labeled_perc * self.PVC_ECG.shape[0]), replace=False)
        self.NSR_subsample_idx = np.random.choice(range(self.NSR_ECG.shape[0]), round(self.labeled_perc * self.NSR_ECG.shape[0]), replace=False)

        self.AF_ECG = self.AF_ECG[self.AF_subsample_idx]
        self.PVC_ECG = self.PVC_ECG[self.PVC_subsample_idx]
        self.NSR_ECG = self.NSR_ECG[self.NSR_subsample_idx]
