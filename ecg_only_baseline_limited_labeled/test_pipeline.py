...
def test_epoch(PPG_model, test_loader, test_ds_name, baseline=False, patient_info_path=None):
    with torch.no_grad():
        ...
        if patient_info_path is not None:
            ...
            np.save(f'/labs/hulab/stark_stuff/ppg_ecg_project/data/{test_ds_name}_ecgonlylimitedlabel_aurocs22.npy', all_bt_aurocs)
            np.save(f'/labs/hulab/stark_stuff/ppg_ecg_project/data/{test_ds_name}_ecgonlylimitedlabel_auprcs22.npy', all_bt_auprcs)
            np.save(f'/labs/hulab/stark_stuff/ppg_ecg_project/data/{test_ds_name}_ecgonlylimitedlabel_targets22.npy', all_bt_targets)
            np.save(f'/labs/hulab/stark_stuff/ppg_ecg_project/data/{test_ds_name}_ecgonlylimitedlabel_pred_probs22.npy', all_bt_pred_probs)
